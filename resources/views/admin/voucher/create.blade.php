...
{{-- <script>
    document.querySelector('form').addEventListener('submit', function(event) {
        // Chuyển đổi giá trị start_date và end_date trước khi gửi
        const startDateInput = document.getElementById('start_date');
        const endDateInput = document.getElementById('end_date');

        if (startDateInput && endDateInput) {
            // Chuyển đổi định dạng datetime-local thành datetime chuẩn
            startDateInput.value = startDateInput.value.replace('T', ' ') + ':00'; // thêm giây
            endDateInput.value = endDateInput.value.replace('T', ' ') + ':00'; // thêm giây
        }
    });
</script> --}}
